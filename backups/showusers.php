...
if ( $adminloggedIn ) {
    if ( $db = new PDO( 'sqlite:' . $dbName . '-users.db' ) ) {
        if ( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $hardDemo ) {
            ...
            if ( $_POST[ 'del' ] == 1 ) {
                ...
                if ( is_numeric( $_POST[ 'accid' ] ) )
                    ...
                    $statement = $db->prepare( "DELETE FROM FB WHERE username='" . $uname . "'" );
                if ( $statement ) {
                    $statement->execute();
                } else {
                    showHTML( "User Deletion failed!" );
                }
                if ( $db2 = new PDO( 'sqlite:' . $dbName . '-crons.db' ) ) {
                    $statement = $db2->prepare( "DELETE FROM Crons WHERE user LIKE '% (" . $uname . ")'" );
                    if ( $statement ) {
                        $statement->execute();
                    } else {
                        showHTML( "User CRONs Deletion failed!" );
                    }
                }
                if ( $db3 = new PDO( 'sqlite:' . $dbName . '-presets.db' ) ) {
                    $statement = $db3->prepare( "DELETE FROM Presets WHERE username='" . $uname . "'" );
                    if ( $statement ) {
                        $statement->execute();
                    } else {
                        showHTML( "User Presets Deletion failed!" );
                    }
                }
            } else {
                if ( is_numeric( $_POST[ 'accid' ] ) )
        ...
        $message .= '<div>
            Search: <input id="filter" type="text" style="width:auto;" value="' . $_GET['search'] . '" />
        ...
        foreach ( $tempData as $s ) {
            ...
            if ( $hardDemo && $tempID )
                $message .= "src='?proxyurl=" . urlencode( encrypt( "http://graph.facebook.com/" . $GLOBALS[ '__FBAPI__' ] . "/" . $tempID . "/picture?redirect=1&height=32&type=normal&width=32" ) ) . "'";
            elseif ( !$hardDemo && ( $s[ 'userid' ] != "" ) )
                $message .= "src='http://graph.facebook.com/" . $GLOBALS[ '__FBAPI__' ] . "/" . $s[ 'userid' ] . "/picture?redirect=1&height=32&type=normal&width=32'";  
            ...
            $message .= "<td data-value='" . ( $userOptions[ 'userDisabled' ] ? ( $userOptions[ 'userDisabled' ] == 1 ? 'Disabled' : 'Awaiting Approval' ) : 'Enabled' ) . "'><img src='img/" . ( $userOptions[ 'userDisabled' ] ? ( $userOptions[ 'userDisabled' ] == 1 ? 'disabled' : 'awaiting' ) : 'enabled' ) . ".png' title='Click to " . ( $userOptions[ 'userDisabled' ] ? ( $userOptions[ 'userDisabled' ] == 1 ? 'enable (Disable Reason: ' . $userOptions[ 'disableReason' ] . ')' : 'approve' ) : 'disable' ) . " User' onclick='Accounts(event,\"$userIdentity\",\"$uname\"," . ( $userOptions[ 'userDisabled' ] ? '0' : '-1' ) . ")'>";
            $message .= "&nbsp;&nbsp;<img src='img/delete.png' width='16px' title='Delete User' onclick='Accounts(event,\"$userIdentity\",\"$uname\",1)'>";
        }
        $message .= "</tbody>
