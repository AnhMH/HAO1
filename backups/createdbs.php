...
if ( !file_exists( 'params.php' ) && !isset( $_POST[ 'form_id' ] ) && !file_exists( 'config.php' ) ) {
    showHTML( include_once( 'includes/install.php' ), "Welcome to FB Multi Page/Group Poster" );
}
...
if ( !file_exists( $dbName . '-presets.db' ) ) {
    if ( $db = new PDO( 'sqlite:' . $dbName . '-presets.db' ) ) {
        $statement = $db->prepare( "CREATE TABLE Presets (userid TEXT, username TEXT COLLATE NOCASE, presetname TEXT, posttype TEXT, params TEXT, groups TEXT)" );
        if ( $statement ) {
            $statement->execute();
        }
        $db = null;
    } else {
        showHTML( "Error - Unable to create presets database. Exiting..." );
    }
}
?>